...
static LCD_ERRORSTATE_t LCD_PowerON_Proc (void){
	...
	UserREQ.State=LCD_REQ_busy;
	u8 idx;
	GPIOPIN_t LCD_pinHandler;

	for (idx=0;idx<_LCD_PIN_NUMNER;idx++)
	{
		LCD_pinHandler.gpioMODE=GPIO_OUTPUT_PP;
		LCD_pinHandler.gpioSPEED=GPIO_VERYHIGHSPEED;
		LCD_pinHandler.gpioPIN=LCDs[idx].pin;
		LCD_pinHandler.gpioPORT=LCDs[idx].port;
		localreturn=GPIO_InitPin(&LCD_pinHandler);
	}
...
static LCD_ERRORSTATE_t LCD_WriteCommand (u8 command){
	...
	u8 LCD_pinstate;
	if (LCD_PinEnable==Enable_OFF){
		localreturn=GPIO_SetPinValue(LCDs[RS].port,LCDs[RS].pin,LOGIC_LOW);

		localreturn |= GPIO_SetPinValue(LCDs[RW].port,LCDs[RW].pin,LOGIC_LOW);

		for (idx=0;(idx<(_LCD_PIN_NUMNER -control_pins_Num));idx++){

			LCD_pinstate=(command & (1 << idx)) ? LOGIC_HIGH : LOGIC_LOW;

			localreturn=GPIO_SetPinValue(LCDs[idx].port,LCDs[idx].pin,LCD_pinstate);
...
static LCD_ERRORSTATE_t LCD_WriteData (u8 data){
	...
	u8 LCD_pinstate;
	if (LCD_PinEnable==Enable_OFF){
		localreturn=GPIO_SetPinValue(LCDs[RS].port,LCDs[RS].pin,LOGIC_HIGH);

		localreturn |= GPIO_SetPinValue(LCDs[RW].port,LCDs[RW].pin,LOGIC_LOW);
		for (idx=0;(idx<(_LCD_PIN_NUMNER -control_pins_Num));idx++){

			LCD_pinstate=((data & (1 << idx))) ? LOGIC_HIGH : LOGIC_LOW;
...
static LCD_ERRORSTATE_t LCD_EnableControl (u32 Enable){
	...
	localreturn =GPIO_SetPinValue(LCDs[En].port,LCDs[En].pin,Enable);
...
static LCD_ERRORSTATE_t LCD_disableControl (u32 Enable){
	...
	localreturn =GPIO_SetPinValue(LCDs[En].port,LCDs[En].pin,Enable_OFF);
